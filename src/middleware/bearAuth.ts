...
interface DecodedToken {
    book_id: number;           // Unique identifier for the user
    title: string;        // User's first name
    author: string;         // User's last name
    genre: string; 
    published_year: number;
    isbn: number;
    price: number;
    stock_quantity: number;            // User's email address
    user_type: 'admin' | 'user'; // User's role in the system
...
export const authMiddleware = async (c: Context, next: Next, requiredRole: UserRole) => {
    ...
    if (requiredRole === "both") {
        ...
        if (decoded.user_type === "admin" || decoded.user_type === "user") {
